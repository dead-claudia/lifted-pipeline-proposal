...
This has already been asked, and here's why:

**Pros:**

(This mostly benefits memory, but it can help speed, too.)
...
(And some counter-arguments for some of them.)
...
2. The operator looks a bit foreign and/or weird.
    - Yeah...I initially used `>=>` in the mailing list, and `>:>`/`<:<` here, but I was mostly looking for an operator that a) didn't conflict with existing syntax (`f <<- g` conflicts with `f << -g`, for example), and b) indicated some sort of flowing. If you have a better idea, I'm all ears.
    - I did adjust my original proposal to go both ways, since they each have their merits.
3. Why syntax? Why can't this be a normal function?
    - There are certain memory optimizations that can only be realistically done *with* syntax.
